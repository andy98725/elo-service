...
func TestMatchReportsResult(t *testing.T) {
	...
	guest1Token := guest1["token"].(string)
	guest1ID := guest1["id"].(string)
	guest2 := DoReq(t, "POST", fmt.Sprintf("%s/guest/login", *baseURL), map[string]string{"displayName": "guest2"}, "", http.StatusOK)
	guest2Token := guest2["token"].(string)
	guest2ID := guest2["id"].(string)
	...
	if queueResponse["players_in_queue"].(float64) != 0 {
		t.Fatalf("expected 0 players in queue, got %d", queueResponse["players_in_queue"].(int64))
	}
	...
	waitForMatchFound := func(t *testing.T, wsConn interface {
		...
		for {
			...
			case "error":
				...
				t.Fatalf("%s: received error: %v", name, reason)
			default:
				continue
			}
		}
	...
	DoReq(t, "POST", fmt.Sprintf("%s:8080/join", serverAddr1), guest1ID, "", http.StatusOK)
	DoReq(t, "POST", fmt.Sprintf("%s:8080/join", serverAddr1), guest2ID, "", http.StatusOK)

	time.Sleep(1 * time.Second)

	logs := DoReq(t, "GET", fmt.Sprintf("%s:9999/logs", serverAddr1), nil, "", http.StatusOK)
	t.Log(logs)
